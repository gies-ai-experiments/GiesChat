  ...
  return (
    <span
      className={cn(
        ...
        online === true ? 'bg-[#22C55E]' : 'border border-border-light',
      )}
        ...
        {participants.map((participant) => (
          <li key={participant.userId} className="flex items-center gap-2 py-1.5">
            ...
            {participant.role === 'owner' && (
              <span className="ml-auto shrink-0 rounded-[10px] bg-surface-tertiary px-[7px] py-px text-[10px] font-bold uppercase text-text-secondary">
...
export default function ParticipantsBar({ participants }: { participants: TRoomParticipant[] }) {
  ...
  return (
    <div
      className="flex items-center gap-2 border-b border-border-light bg-surface-primary px-4 py-2"
      aria-label={localize('com_ui_brainstorm_members')}
    >
      {participants.map((participant) => (
        <div key={participant.userId} className="relative" title={participant.name}>
          <div className="flex size-8 items-center justify-center rounded-full bg-surface-tertiary text-xs font-semibold text-text-primary">
