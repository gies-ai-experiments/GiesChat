...
function buildInstructions() {
  return [
    ...
    '',
    'Workflow:',
    '1. Brainstorm first. You MUST ask 2-3 idea-specific brainstorming questions before building anything. Generate the questions whose answers would most change what you build for THIS specific idea — derive them fresh from what the student described, never from a stock template. Good questions resolve real forks in their idea (for a workout tracker: just for them or shared with friends? for a store: how should payments work? for a dashboard: where does the data come from?). Skip anything they already told you. Ask them ONE AT A TIME: a single question, wait for the answer, then choose the next question based on that answer. Never batch multiple questions into one message. You MUST NOT call create_app_from_prompt until the questions are answered unless the student explicitly asks you to build it directly.',
